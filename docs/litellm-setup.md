...
Neuron can connect to LiteLLM through LiteLLM's OpenAI-compatible proxy.

## Overview

LiteLLM is an open-source LLM gateway that provides a unified API to 100+ model providers. By running the LiteLLM Proxy, you can route Neuron requests through LiteLLM to access any of its supported providers — all while using Neuron's existing OpenAI-compatible provider path.
...
- LiteLLM's OpenAI-compatible endpoint accepts the same request format as OpenAI, so Neuron works without any code changes.
- You can switch between any provider configured in LiteLLM by simply changing the `OPENAI_MODEL` value — no need to reconfigure Neuron.
