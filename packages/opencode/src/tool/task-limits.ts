...
export class LimitError extends Schema.TaggedErrorClass<LimitError>()("TaskLimitExceeded", {
  kind: Schema.Literals(["depth", "direct", "total"]),
...
export const check = Effect.fn("TaskLimits.check")(function* (
  ...
  if (depth + 1 > MAX_DEPTH) {
    return yield* new LimitError({
      kind: "depth",
      message: `Sub-agent depth cap reached (max ${MAX_DEPTH}). Delegating any deeper would stall the run; do the work in the current agent instead.`,
    })
  }
  ...
  if (runningDirect + 1 > MAX_DIRECT_CHILDREN) {
    return yield* new LimitError({
      kind: "direct",
      message: `Concurrent sub-agent cap reached (max ${MAX_DIRECT_CHILDREN} running direct children per session). Wait for an existing sub-agent to finish or merge two tasks into one.`,
    })
  }
  ...
  if (subAgents + 1 > MAX_TREE_TOTAL) {
    return yield* new LimitError({
      kind: "total",
      message: `Total sub-agent cap reached for this session tree (max ${MAX_TREE_TOTAL}). Finish some tasks before spawning more.`,
    })
  }
})
