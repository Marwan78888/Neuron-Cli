...
export const check = Effect.fn("TaskLimits.check")(function* (sessions: Session.Interface, parentID: SessionID) {
  ...
  const direct = yield* sessions.children(parentID)
  if (direct.length + 1 > MAX_DIRECT_CHILDREN) {
    return new LimitError(
      "direct",
      `Concurrent sub-agent cap reached (max ${MAX_DIRECT_CHILDREN} direct children per session). Wait for an existing sub-agent to finish or merge two tasks into one.`,
    )
  }

  const rootID = yield* walkRoot(sessions, parentID)
  const total = yield* countTree(sessions, rootID)
  if (total + 1 > MAX_TREE_TOTAL) {
    return new LimitError(
      "total",
...
const countTree = Effect.fn("TaskLimits.countTree")(function* (sessions: Session.Interface, rootID: SessionID) {
  ...
  while (queue.length > 0) {
    ...
    const kids = yield* sessions.children(current).pipe(Effect.catchCause(() => Effect.succeed<Array<{ id: string }>>([])))
