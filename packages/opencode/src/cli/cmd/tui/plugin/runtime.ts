...
import { LEGACY_PROJECT_CONFIG_DIR, PROJECT_CONFIG_DIR } from "@/config/paths"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === PROJECT_CONFIG_DIR || path.basename(source_dir) === LEGACY_PROJECT_CONFIG_DIR
        ? path.join(source_dir, "themes")
        : path.join(source_dir, PROJECT_CONFIG_DIR, "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, PROJECT_CONFIG_DIR, "tui.json"),
  }
}
