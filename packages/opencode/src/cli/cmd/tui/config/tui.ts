...
import { PROJECT_CONFIG_DIRS } from "@/config/paths"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any project config
  ...
  // 4. Project config directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => PROJECT_CONFIG_DIRS.some((name) => dir.endsWith(name)) || dir === Flag.OPENCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!PROJECT_CONFIG_DIRS.some((name) => dir.endsWith(name)) && dir !== Flag.OPENCODE_CONFIG_DIR) continue
