...
import neuron from "./theme/neuron.json" with { type: "json" }
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "neuron")
        draft.active = typeof active === "string" ? active : "neuron"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.neuron, store.mode)
    })
