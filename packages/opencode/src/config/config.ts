...
import { PROJECT_CONFIG_DIRS } from "./paths"
import { Npm } from "@/npm"
...
const InfoSchema = Schema.Struct({
  ...
  search: Schema.optional(
    Schema.Struct({
      provider: Schema.optional(Schema.Literals(["perplexity", "brave"])).annotate({
        description: "Preferred external search provider",
      }),
    ...
    description: "Search configuration used by Neuron for external browsing workflows",
  }),
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under project config plugin folders are already local files, so ConfigPlugin.load
