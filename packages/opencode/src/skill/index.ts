...
const add = Effect.fnUntraced(function* (state: State, match: string, bus: Bus.Interface) {
  ...
  const parsed = Info.pick({ name: true, description: true }).safeParse({
    name: typeof raw.name === "string" && raw.name.trim() ? raw.name.trim() : path.basename(path.dirname(match)),
    description:
      (typeof raw.description === "string" && raw.description.trim()) ||
      (typeof raw.when_to_use === "string" && raw.when_to_use.trim()) ||
      fallbackDescription(md.content),
  })
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.OPENCODE_DISABLE_EXTERNAL_SKILLS) {
    ...
    const workspaceSkills = path.join(worktree, "skills")
    if (yield* fsys.isDir(workspaceSkills)) {
      yield* scan(state, workspaceSkills, SKILL_PATTERN, { dot: true, scope: "workspace" })
    }
  }
