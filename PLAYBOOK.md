# Neuron Local Agent Playbook

This playbook is a practical guide to run Neuron with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Neuron starts directly.
...
Or let Neuron recommend the best local model for your goal:
...
```powershell
Get-Content .\.neuron-profile.json
```
...
- Keep `.neuron-profile.json` local (already gitignored).
