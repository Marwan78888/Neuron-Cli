...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets Neuron handle permission prompts automatically — Neuron checks each tool call for risky actions and prompt injection before executing. Actions Neuron identifies as safe are executed, while actions Neuron identifies as risky are blocked and Neuron may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. Neuron can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
...
export function AutoModeOptInDialog(t0: Props) {
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column" gap={1}><Text>{AUTO_MODE_DESCRIPTION}</Text><Link url="https://github.com/Marwan78888/Neuron-Cli/blob/main/SECURITY.md" /></Box>;
    $[4] = t3;
  } else {
    t3 = $[4];
  }
