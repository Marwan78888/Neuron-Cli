...
// RUNE — Neuron Signature (🧠 Thinking, default)
const RUNE = ['∙', '⊹', '⊠', '⊞', '⊟', '⊡']

// ARROW — Executing / movement (⚙️)
const ARROW = ['←', '↖', '↑', '↗', '→', '↘', '↓', '↙']

// BAR — Progress (📊 Progress)
const BAR = ['▏', '▎', '▍', '▌', '▋', '▊', '▉', '█', '▉', '▊', '▋', '▌', '▍', '▎']

// BINARY — Heavy processing (🧪)
const BINARY = ['010', '001', '110', '100', '011', '101', '110', '011']

export function getDefaultCharacters(
  mode?: SpinnerMode,
  spinnerType?: 'rune' | 'arrow' | 'bar' | 'binary',
): string[] {
  if (spinnerType) {
    switch (spinnerType) {
      case 'rune':
  ...
  // Select animation frames based on spinner mode
  switch (mode) {
    case 'requesting':
      // Requesting: use ARROW for execution/movement
      return ARROW
    case 'tool-use':
    case 'tool-input':
      // Heavy processing: use BINARY for complex operations
      return BINARY
    case 'responding':
      // Progress: use BAR for feedback
      return BAR
    case 'thinking':
    default:
      // Thinking: use RUNE (default Neuron signature)
      return RUNE
  }
}
