...
export function SpinnerGlyph(t0) {
  ...
  const SPINNER_FRAMES = [...getDefaultCharacters(mode, spinnerType), ...[...getDefaultCharacters(mode, spinnerType)].reverse()];
  ...
  if (mode === 'thinking') {
    return (
      <Box flexWrap="wrap" height={1} width={2}>
        <Text color={toRGBColor(THINKING_COLOR)}>{spinnerChar}</Text>
      </Box>
    );
  }
