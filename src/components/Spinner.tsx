...
// Note: SPINNER_FRAMES is now mode-dependent and computed in SpinnerGlyph
// These constants kept for backward compatibility but not used in the animation loop
...
function SpinnerWithVerbInner({
  ...
  const defaultColor: keyof Theme = mode === 'thinking' ? 'chromeYellow' : 'claude';
  const defaultShimmerColor = mode === 'thinking' ? 'warningShimmer' : 'claudeShimmer';
  ...
  const effectiveTip = contextTipsActive ? undefined : showClearTip && !nextTask ? 'Use /clear to start fresh when switching topics and free up context' : showBtwTip && !nextTask ? "Use /btw to ask a quick side question without interrupting Neuron's current work" : spinnerTip;
