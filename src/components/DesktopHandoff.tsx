...
export function DesktopHandoff(t0) {
    ...
    t2 = () => {
      const performHandoff = async function performHandoff() {
        ...
        if (installStatus.status === "not-installed") {
          setDownloadMessage("The desktop app is not installed.");
          setState("prompt-download");
          return;
        }
        if (installStatus.status === "version-too-old") {
          setDownloadMessage(`The desktop app needs to be updated (found v${installStatus.version}, need v1.1.2396+).`);
          setState("prompt-download");
          return;
        }
        setState("flushing");
        ...
        if (!result.success) {
          setError(result.error ?? "Failed to open the desktop app");
          setState("error");
          return;
        }
        setState("success");
  ...
  if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = {
      checking: "Checking for the desktop app\u2026",
      flushing: "Saving session\u2026",
      opening: "Opening the desktop app\u2026",
      success: "Opening in the desktop app\u2026"
    };
    $[17] = t4;
  } else {
    t4 = $[17];
...
async function _temp2(onDone_0) {
  onDone_0("Session transferred to the desktop app", {
    display: "system"
  });
