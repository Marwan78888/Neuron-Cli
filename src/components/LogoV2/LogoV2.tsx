...
export function LogoV2() {
  ...
  const configuredUserName = getInitialSettings().userName?.trim() ?? "";
  const username = configuredUserName || (showAccountIdentity ? getGlobalConfig().oauthAccount?.displayName ?? "" : "");
  const {
    columns
  } = useTerminalSize();
  let t0;
  ...
  const borderTitle = ` ${color("text", userTheme)("NEURON")} ${color("inactive", userTheme)(`v${version}`)} `;
  const compactBorderTitle = color("text", userTheme)(" NEURON ");
  if (layoutMode === "compact") {
    ...
    if ($[34] === Symbol.for("react.memo_cache_sentinel")) {
      t12 = <Box marginY={1} width="100%"><NeuronBanner /></Box>;
      $[34] = t12;
    } else {
      t12 = $[34];
    }
    ...
    return <><OffscreenFreeze><Box flexDirection="column" borderStyle="round" borderColor="inactive" borderText={t11} paddingX={1} paddingY={1} alignItems="center" width={columns}><Text bold={true}>Welcome to Neuron</Text><Text color="chromeYellow">{"Let's get started."}</Text>{t12}{t13}<Text dimColor={true}>{billingType}</Text><Text dimColor={true}>{agentName ? `@${agentName} · ${truncatedCwd}` : truncatedCwd}</Text></Box></OffscreenFreeze>{t14}{t15}{t16}{t17}{t18}{t19}</>;
  }
  ...
  if ($[46] !== welcomeMessage_0) {
    t18 = <Box marginTop={1} flexDirection="column" alignItems="center" width="100%"><NeuronBanner /><Box marginTop={1} flexDirection="column" alignItems="center"><Text bold={true} color="white">Welcome to Neuron</Text><Text color="chromeYellow">{"Let's get started."}</Text>{welcomeMessage_0 !== "Welcome to Neuron" && <Text dimColor={true}>{welcomeMessage_0}</Text>}</Box></Box>;
    $[46] = welcomeMessage_0;
    $[47] = t18;
  } else {
    t18 = $[47];
