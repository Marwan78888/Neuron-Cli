...
function getPreferredProjectConfigPath(
  dir: string,
  subdir: 'skills' | 'commands',
): string {
  return pickPreferredExistingPath(
    getProjectConfigSubdirPaths(dir, subdir),
...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.neuron', dir)
    case 'userSettings':
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const managedSkillsDir = getPreferredProjectConfigPath(
      getManagedFilePath(),
      'skills',
    )
      ...
      const additionalSkillsNested = await Promise.all(
        additionalDirs.map(dir =>
          loadSkillsFromSkillsDir(
            getPreferredProjectConfigPath(dir, 'skills'),
            'projectSettings',
          ),
        ),
      )
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = getPreferredProjectConfigPath(currentDir, 'skills')
