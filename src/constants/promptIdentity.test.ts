...
test('CLI identity prefixes describe Neuron instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('Neuron')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('Neuron')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes Neuron instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('Neuron')
...
test('built-in agent prompts describe Neuron instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('Neuron')
  ...
  expect(generalPrompt).toContain('Neuron')
  ...
  expect(explorePrompt).toContain('Neuron')
  ...
  expect(guidePrompt).toContain('Neuron')
  expect(guidePrompt).toContain('You are the Neuron guide agent.')
  expect(guidePrompt).toContain('**Neuron** (the CLI tool)')
