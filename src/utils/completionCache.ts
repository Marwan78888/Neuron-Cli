...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const cliConfigDir = getClaudeConfigHomeDir()

  if (shell.endsWith('/zsh') || shell.endsWith('/zsh.exe')) {
    const cacheFile = join(cliConfigDir, 'completion.zsh')
    return {
      name: 'zsh',
  ...
  if (shell.endsWith('/bash') || shell.endsWith('/bash.exe')) {
    const cacheFile = join(cliConfigDir, 'completion.bash')
    return {
      name: 'bash',
  ...
  if (shell.endsWith('/fish') || shell.endsWith('/fish.exe')) {
    ...
    const cacheFile = join(cliConfigDir, 'completion.fish')
    return {
      name: 'fish',
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: ${formatCliCommand(`completion ${shell.shellFlag} > ${shell.cacheFile}`)}`)}${EOL}`
  }
  ...
  const cliBin = process.argv[1] || getCliBinaryName()
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: ${formatCliCommand(`completion ${shell.shellFlag} > ${shell.cacheFile}`)}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes(`${getCliBinaryName()} completion`) ||
...
/**
 * Regenerate cached shell completion scripts in the CLI config directory.
