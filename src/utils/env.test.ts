...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'neuron-env-test-'))
...
test('getGlobalClaudeFile: new install returns .neuron.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.neuron.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .neuron.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.neuron.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.neuron.json'))
})
