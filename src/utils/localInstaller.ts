...
function getCandidateLocalBinaryPaths(localInstallDir: string): string[] {
  return [
    join(localInstallDir, 'node_modules', '.bin', 'neuron'),
...
export function isManagedLocalInstallationPath(execPath: string): boolean {
  ...
  return (
    normalizedExecPath.includes('/.neuron/local/node_modules/') ||
    normalizedExecPath.includes('/.claude/local/node_modules/')
  )
}

export function getLocalClaudePath(): string {
  return join(getLocalInstallDir(), 'neuron')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'neuron-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/neuron" "$@"`,
      0o755,
    )
    if (created) {
