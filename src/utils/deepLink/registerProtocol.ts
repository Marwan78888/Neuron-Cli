 ...
 * native installer's stable symlink (~/.local/bin/<cli>) which survives
...
async function resolveClaudePath(): Promise<string> {
  const candidatePaths = Array.from(
    new Set([
      path.join(getUserBinDir(), getCliExecutableName()),
      path.join(getUserBinDir(), getLegacyClaudeExecutableName()),
    ]),
  )

  for (const stablePath of candidatePaths) {
    try {
 ...
 * a cached flag in ~/.neuron.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.claude (per-machine, not synced) rather than ~/.neuron.json (can sync).
