...
function getInstalledExecutableName(platform: string): string {
  return getCliExecutableName(platform)
}

function getBaseDirectories() {
  const platform = getPlatform()
  const executableName = getInstalledExecutableName(platform)
  const dataDirName = getCliBinaryName()

  return {
    // Data directories (permanent storage)
    versions: join(getXDGDataHome(), dataDirName, 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), dataDirName, 'staging'),

    // State directories
    locks: join(getXDGStateHome(), dataDirName, 'locks'),
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/<cli> to the version binary
  await removeDirectoryIfEmpty(executablePath)
  await updateSymlink(executablePath, installPath)
  await removeLegacyNativeExecutableSymlink(executablePath)
...
async function removeLegacyNativeExecutableSymlink(
  currentExecutablePath: string,
): Promise<void> {
  const platform = getPlatform()
  const legacyExecutablePath = join(
    getUserBinDir(),
    getLegacyClaudeExecutableName(platform),
  )

  if (legacyExecutablePath === currentExecutablePath) {
    return
  }

  try {
    if (await isNpmSymlink(legacyExecutablePath)) {
      logForDebugging(
        `Skipping removal of ${legacyExecutablePath} - appears to be npm-managed`,
      )
      return
    }

    await unlink(legacyExecutablePath)
    logForDebugging(`Removed legacy native symlink at ${legacyExecutablePath}`)
  } catch (error) {
    if (!isENOENT(error)) {
      logError(
        new Error(`Failed to remove legacy symlink ${legacyExecutablePath}: ${error}`),
      )
    }
  }
}
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  const candidatePaths = Array.from(
    new Set([
      dirs.executable,
      join(getUserBinDir(), getLegacyClaudeExecutableName(getPlatform())),
    ]),
  )
  ...
  for (const candidatePath of candidatePaths) {
    try {
      ...
      if (await isNpmSymlink(candidatePath)) {
        logForDebugging(
          `Skipping removal of ${candidatePath} - appears to be npm-managed`,
        )
        continue
      }
      ...
      logForDebugging(`Removed native symlink at ${candidatePath}`)
    } catch (error) {
      if (isENOENT(error)) {
        continue
      }
      logError(new Error(`Failed to remove native symlink ${candidatePath}: ${error}`))
    }
  }
}
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const candidateBaseNames = Array.from(
        new Set([getCliBinaryName(), 'claude']),
      )

      for (const candidateBaseName of candidateBaseNames) {
        if (
          await tryRemove(join(globalPrefix, `${candidateBaseName}.cmd`), 'bin script')
        ) {
        ...
        if (
          await tryRemove(
            join(globalPrefix, `${candidateBaseName}.ps1`),
            'PowerShell script',
          )
        ) {
          manuallyRemoved = true
        ...
        if (
          await tryRemove(join(globalPrefix, candidateBaseName), 'bin executable')
        ) {
          manuallyRemoved = true
        }
      }
      ...
      for (const candidateBaseName of candidateBaseNames) {
        if (
          await tryRemove(join(globalPrefix, 'bin', candidateBaseName), 'bin symlink')
        ) {
          manuallyRemoved = true
        }
      }
