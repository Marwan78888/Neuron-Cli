...
export const PROFILE_FILE_NAME = '.neuron-profile.json'
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  // The legacy single-profile file (~/.neuron-profile.json) is a
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.NEURON_PROFILE_GOAL),
    processEnv,
...
export async function applySavedProfileToCurrentSession(options: {
  ...
  const nextEnv = await buildLaunchEnv({
    ...
    goal: normalizeRecommendationGoal(processEnv.NEURON_PROFILE_GOAL),
    processEnv: baseEnv,
    getOllamaChatBaseUrl,
