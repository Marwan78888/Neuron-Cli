...
import { getProjectConfigSubdirPaths } from './projectInstructions.js'
...
function selectPreferredConfigSubdir(
  baseDir: string,
  subdir: ClaudeConfigDirectory,
): string {
  const candidates = getProjectConfigSubdirPaths(baseDir, subdir)
 ...
 * collecting all .neuron directories along the way, with legacy .claude
 * directories still supported as a fallback.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.neuron/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .neuron/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = selectPreferredConfigSubdir(current, subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = selectPreferredConfigSubdir(getManagedFilePath(), subdir)
