    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.neuron.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.neuron.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.neuron.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.neuron.json is user-writable and cannot be trusted.
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from '${formatCliCommand('setup-token')}' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via '${formatCliCommand('auth login')}'.`,
    }
  }
  ...
  return {
      ...
      `Please log in with the correct organization: ${formatCliCommand('auth login')}`,
  }
}
