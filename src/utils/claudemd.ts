  ...
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        const dotClaudePath = getProjectNestedInstructionFilePath(
          dir,
          existsSync,
        )
        result.push(
        ...
        const rulesDir = pickPreferredExistingPath(
          getProjectRulesDirPaths(dir),
          existsSync,
        )
        result.push(
          ...(await processMdRules({
      ...
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = getLocalInstructionFilePath(dir, existsSync)
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
...
export async function getMemoryFilesForNestedDirectory(
  ...
  if (isSettingSourceEnabled('projectSettings')) {
    ...
    const dotClaudePath = getProjectNestedInstructionFilePath(dir, existsSync)
    result.push(
      ...(await processMemoryFile(
        dotClaudePath,
        'Project',
...
export function isMemoryFilePath(filePath: string): boolean {
  ...
  if (isProjectInstructionFileName(name) || isLocalInstructionFileName(name)) {
    return true
  }
  ...
  if (
    ...
    (filePath.includes(`${sep}.claude${sep}rules${sep}`) ||
      filePath.includes(`${sep}.neuron${sep}rules${sep}`))
  ) {
    return true
  }
