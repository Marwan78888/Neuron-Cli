...
import { formatCliCommand, getCliBinaryName, getCliDisplayName } from './utils/cliIdentity.js';
  ...
  if (feature('SSH_REMOTE') && _pendingSSH) {
    ...
    if (rawCliArgs[0] === 'ssh' && rawCliArgs[1] && !rawCliArgs[1].startsWith('-')) {
      ...
      if (rest.includes('-p') || rest.includes('--print')) {
        process.stderr.write(`Error: headless (-p/--print) mode is not supported with ${formatCliCommand('ssh')}\n`);
...
async function run(): Promise<CommanderCommand> {
  ...
  program.hook('preAction', async thisCommand => {
    ...
    if (!isEnvTruthy(process.env.CLAUDE_CODE_DISABLE_TERMINAL_TITLE)) {
      process.title = getCliBinaryName();
    }
  ...
  program.name(getCliBinaryName()).description(`${getCliDisplayName()} - starts an interactive session by default, use -p/--print for non-interactive output`).argument('[prompt]', 'Your prompt', String)
    ...
    .option('--plugin-dir <path>', 'Load plugins from a directory for this session only (repeatable: --plugin-dir A --plugin-dir B)', (val: string, prev: string[]) => [...prev, val], [] as string[]).option('--disable-slash-commands', 'Disable all skills', () => true).option('--chrome', 'Enable Chrome integration').option('--no-chrome', 'Disable Chrome integration').option('--file <specs...>', 'File resources to download at startup. Format: file_id:relative_path (e.g., --file file_abc:doc.txt file_def:img.png)').action(async (prompt, options) => {
      ...
      if (prompt === 'code') {
        ...
        console.warn(chalk.yellow(`Tip: You can launch ${getCliDisplayName()} with just \`${getCliBinaryName()}\``));
        prompt = undefined;
      }
      ...
      if (options.continue) {
        ...
        if (!targetSessionId) {
          ...
          if (sessions.length === 0) {
            ...
            return await exitWithMessage(root, `Assistant installed in ${installedDir}. The daemon is starting up — run \`${formatCliCommand('assistant')}\` again in a few seconds to connect.`, {
              exitCode: 0,
        ...
        if (remote !== null) {
          ...
          if (!isRemoteTuiEnabled && !hasInitialPrompt) {
            return await exitWithError(root, `Error: --remote requires a description.\nUsage: ${formatCliCommand('--remote "your task description"')}`, () => gracefulShutdown(1));
          }
          ...
          if (!isRemoteTuiEnabled) {
            ...
            process.stdout.write(`Resume with: ${formatCliCommand(`--teleport ${createdSession.id}`)}\n`);
            await gracefulShutdown(0);
            process.exit(0);
          }
              ...
              if (repoValidation.status === 'mismatch' || repoValidation.status === 'not_in_repo') {
                ...
                if (sessionRepo) {
                  ...
                  if (existingPaths.length > 0) {
                    ...
                    throw new TeleportOperationError(`You must run ${formatCliCommand(`--teleport ${teleport}`)} from a checkout of ${sessionRepo}.`, chalk.red(`You must run ${formatCliCommand(`--teleport ${teleport}`)} from a checkout of ${chalk.bold(sessionRepo)}.\n`));
                  }
                }
    ...
    }).version(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Neuron)`, '-v, --version', 'Output the version number');
  ...
  mcp.command('serve').description(`Start the ${getCliDisplayName()} MCP server`).option('-d, --debug', 'Enable debug mode', () => true).option('--verbose', 'Override verbose mode setting from config', () => true).action(async ({
    debug,
    verbose
  }: {
    debug?: boolean;
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('server').description(`Start a ${getCliDisplayName()} session server`).option('--port <number>', 'HTTP port', '0').option('--host <string>', 'Bind address', '0.0.0.0').option('--auth-token <token>', 'Bearer token for auth').option('--unix <path>', 'Listen on a unix domain socket').option('--workspace <dir>', 'Default working directory for sessions that do not specify cwd').option('--idle-timeout <ms>', 'Idle timeout for detached sessions in ms (0 = never expire)', '600000').option('--max-sessions <n>', 'Maximum concurrent sessions (0 = unlimited)', '32').action(async (opts: {
      ...
      if (existing) {
        process.stderr.write(`A ${getCliBinaryName()} server is already running (pid ${existing.pid}) at ${existing.httpUrl}\n`);
        process.exit(1);
      }
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('open <cc-url>').description(`Connect to a ${getCliDisplayName()} server (internal — use cc:// URLs)`).option('-p, --print [prompt]', 'Print mode (headless)').option('--output-format <format>', 'Output format: text, json, stream-json', 'text').action(async (ccUrl: string, opts: {
  ...
  const pluginCmd = program.command('plugin').alias('plugins').description('Manage Neuron plugins').configureHelp(createSortedHelpConfig());
  ...
  if (feature('KAIROS')) {
    program.command('assistant [sessionId]').description('Attach the REPL as a client to a running bridge session. Discovers sessions via API if no sessionId given.').action(() => {
      ...
      process.stderr.write(`Usage: ${formatCliCommand('assistant [sessionId]')}\n\n` + 'Attach the REPL as a viewer client to a running bridge session.\n' + 'Omit sessionId to discover and pick from available sessions.\n');
      process.exit(1);
    });
  }

  // Doctor command - check installation health
  program.command('doctor').description(`Check the health of your ${getCliDisplayName()} installation. Note: The workspace trust dialog is skipped and stdio servers from .mcp.json are spawned for health checks. Only use this command in directories you trust.`).action(async () => {
    const [{
      doctorHandler
    }, {
      createRoot
  ...
  program.command('repair [target]').description(`Repair the current ${getCliDisplayName()} installation. Reinstalls the configured channel by default; use [target] for latest, stable, or a specific version.`).option('--force', 'Force a native reinstall even if the install appears healthy').action(async (target: string | undefined, options: {
    force?: boolean;
  }) => {
    const {
      repair
    } = await import('src/cli/repair.js');
    await repair(target, options);
  });
  ...
  if ("external" === 'ant') {
    program.command('rollback [target]').description(`[internal-only] Roll back to a previous release\n\nExamples:\n  ${formatCliCommand('rollback')}                                    Go 1 version back from current\n  ${formatCliCommand('rollback 3')}                                  Go 3 versions back from current\n  ${formatCliCommand('rollback 2.0.73-dev.20251217.t190658')}        Roll back to a specific version`).option('-l, --list', 'List recent published versions with ages').option('--dry-run', 'Show what would be installed without installing').option('--safe', 'Roll back to the server-pinned safe version (set by oncall during incidents)').action(async (target?: string, options?: {
      list?: boolean;
      dryRun?: boolean;
      safe?: boolean;
    }) => {
  ...
  // claude install
  program.command('install [target]').description(`Install the ${getCliDisplayName()} native build. Use [target] to specify version (stable, latest, or specific version)`).option('--force', 'Force installation even if already installed').action(async (target: string | undefined, options: {
    force?: boolean;
  }) => {
    const {
      installHandler
...
Examples:
  $ ${formatCliCommand('export 0 conversation.txt')}                Export conversation at log index 0
  $ ${formatCliCommand('export <uuid> conversation.txt')}           Export conversation by session ID
  $ ${formatCliCommand('export input.json output.txt')}             Render JSON log file to text
  $ ${formatCliCommand('export <uuid>.jsonl output.txt')}           Render JSONL session file to text`).action(async (source: string, outputFile: string) => {
      const {
        exportHandler
      } = await import('./cli/handlers/ant.js');
