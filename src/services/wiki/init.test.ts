...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'neuron-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    join('.neuron', 'wiki', 'schema.md'),
    join('.neuron', 'wiki', 'index.md'),
    join('.neuron', 'wiki', 'log.md'),
    join('.neuron', 'wiki', 'pages', 'architecture.md'),
  ])
