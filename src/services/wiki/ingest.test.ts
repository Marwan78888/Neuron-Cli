...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'neuron-wiki-ingest-'))
  tempDirs.push(dir)
  return dir
}
...
test('ingestLocalWikiSource creates a source note and updates log/index', async () => {
  ...
  expect(result.sourceNote.startsWith('.neuron/wiki/sources/')).toBe(true)
  ...
  expect(index).toContain(result.sourceNote.replace('.neuron/wiki/', './'))
})
