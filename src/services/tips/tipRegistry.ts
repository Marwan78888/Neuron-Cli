...
const externalTips: Tip[] = [
  {
    id: 'new-user-warmup',
    content: async () =>
      `Start with small features or bug fixes, tell Neuron to propose a plan, and verify its suggested edits`,
    cooldownSessions: 3,
    async isRelevant() {
  ...
  },
  {
    id: 'git-worktrees',
    content: async () =>
      'Use git worktrees to run multiple Neuron sessions in parallel.',
    cooldownSessions: 10,
    isRelevant: async () => {
      try {
        const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      'Running multiple Neuron sessions? Use /color and /rename to tell them apart at a glance.',
    cooldownSessions: 10,
    isRelevant: async () => {
  ...
  {
    id: 'memory-command',
    content: async () => 'Use /memory to view and manage Neuron memory',
    cooldownSessions: 15,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      'Set NEURON_CODE_USE_POWERSHELL_TOOL=1 to enable the PowerShell tool (preview)',
    cooldownSessions: 10,
    isRelevant: async () =>
      getPlatform() === 'windows' &&
  ...
  },
  {
    id: 'prompt-queue',
    content: async () =>
      'Hit Enter to queue up additional messages while Neuron is working.',
    cooldownSessions: 5,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      'Send messages to Neuron while it works to steer Neuron in real-time',
    cooldownSessions: 20,
    isRelevant: async () => true,
  },
  {
    id: 'todo-list',
    content: async () =>
      'Ask Neuron to create a todo list when working on complex tasks to track progress and remain on track',
    cooldownSessions: 20,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    ...
    content: async () => 'Connect Neuron to your IDE · /ide',
    cooldownSessions: 4,
    async isRelevant() {
  ...
  },
  {
    ...
    content: async () =>
      'Run /install-github-app to tag @neuron right from your Github issues and PRs',
    cooldownSessions: 10,
    isRelevant: async () => !getGlobalConfig().githubActionSetupCount,
  },
  {
    id: 'install-slack-app',
    content: async () => 'Run /install-slack-app to use Neuron in Slack',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'paste-images-mac',
    content: async () =>
      'Paste images into Neuron Code using control+v (not cmd+v!)',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'continue',
    content: async () =>
      'Run neuron --continue or neuron --resume to resume a conversation',
    cooldownSessions: 10,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    id: 'custom-commands',
    content: async () =>
      'Create skills by adding .md files to .neuron/skills/ in your project or ~/.neuron/skills/ for skills that work in any project',
    cooldownSessions: 15,
    async isRelevant() {
      const config = getGlobalConfig()
      return config.numStartups > 10
  ...
  },
  {
    id: 'desktop-app',
    content: async () =>
      'Run Neuron Code locally or remotely using the Neuron desktop app: clau.de/desktop',
    cooldownSessions: 15,
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return `Continue your session in Neuron Code Desktop with ${blue('/desktop')}`
    },
    cooldownSessions: 15,
    isRelevant: async () => {
  ...
  },
  {
    ...
    content: async () =>
      `Your default model setting is Opus Plan Mode. Press ${getShortcutDisplay('chat:cycleMode', 'Chat', 'shift+tab')} twice to activate Plan Mode and plan with Neuron Opus.`,
    cooldownSessions: 2,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return variant === 'copy_b'
        ? `Use ${cmd} for better one-shot answers. Neuron thinks it through first.`
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return variant === 'copy_b'
        ? `For big tasks, tell Neuron to ${blue('use subagents')}. They work in parallel and keep your main thread clean.`
        : `Say ${blue('"fan out subagents"')} and Neuron sends a team. Each one digs deep so nothing gets missed.`
    },
    cooldownSessions: 3,
    isRelevant: async () => {
      if (!is1PApiCustomer()) return false
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return reward
        ? `Share Neuron Code and earn ${claude(formatCreditAmount(reward))} of extra usage · ${claude('/passes')}`
