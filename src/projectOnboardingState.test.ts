...
describe('project onboarding completion', () => {
  test('is incomplete when no repo instruction file exists', async () => {
  ...
  test('is complete when only NEURON.md exists', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'project-onboarding-'))
    await writeFile(join(tempDir, 'NEURON.md'), '# NEURON.md\n')
