...
const OLD_INIT_PROMPT = `Please analyze this codebase and create a NEURON.md file, which will be given to future instances of Neuron to operate in this repository.
...
Usage notes:
- If there's already a NEURON.md or legacy CLAUDE.md, suggest improvements to it.
- When you make the initial NEURON.md, do not repeat yourself and do not include obvious instructions like "Provide helpful error messages to users", "Write unit tests for all new utilities", "Never include sensitive information (API keys, tokens) in code or commits".
...
This file provides guidance to Neuron when working with code in this repository.
\`\`\``

const NEW_INIT_PROMPT = `Set up a minimal AGENTS.md (and optionally NEURON.local.md, skills, and hooks) for this repo. The root project instruction file is loaded into every Neuron session, so it must be concise — only include what Neuron would get wrong without it.
...
- "Which instruction files should /init set up?"
  Options: "Project AGENTS.md" | "Personal NEURON.local.md" | "Both project + personal"
...
- "Also set up skills and hooks?"
  ...
  Description for skills: "On-demand capabilities you or Neuron invoke with \`/skill-name\` — good for repeatable workflows and reference knowledge."
  Description for hooks: "Deterministic shell commands that run on tool events (e.g., format after every edit). Neuron can't skip them."

## Phase 2: Explore the codebase

Launch a subagent to survey the codebase, and ask it to read key files to understand the project: manifest files (package.json, Cargo.toml, pyproject.toml, go.mod, pom.xml, etc.), README, Makefile/build configs, CI config, existing NEURON.md, legacy CLAUDE.md, .neuron/rules/, AGENTS.md, .cursor/rules or .cursorrules, .github/copilot-instructions.md, .windsurfrules, .clinerules, .mcp.json.
...
- Existing .neuron/skills/, .neuron/rules/, and any legacy .claude equivalents
- Formatter configuration (prettier, biome, ruff, black, gofmt, rustfmt, or a unified format script like \`npm run format\` / \`make fmt\`)
- Git worktree usage: run \`git worktree list\` to check if this repo has multiple worktrees (only relevant if the user wants a personal NEURON.local.md)
...
If the user chose personal NEURON.local.md or both: ask about them, not the codebase. Do not mark any options as "recommended" — this is about their personal preferences, not best practices. Examples of questions:
  ...
  - Only if Phase 2 found multiple git worktrees: ask whether their worktrees are nested inside the main repo (e.g., \`.neuron/worktrees/<name>/\`) or siblings/external (e.g., \`../myrepo-feature/\`). If nested, the upward file walk finds the main repo's NEURON.local.md automatically — no special handling needed. If sibling/external, the personal content should live in a home-directory file (e.g., \`~/.neuron/<project-name>-instructions.md\`) and each worktree gets a one-line NEURON.local.md stub that imports it: \`@~/.neuron/<project-name>-instructions.md\`. Never put this import in the project AGENTS.md — that would check a personal reference into the team-shared file.
...
If the repo already has a checked-in root \`NEURON.md\` or legacy \`CLAUDE.md\` and does NOT already have a root \`AGENTS.md\`, do NOT silently create a second root instruction file. In that case, update the existing root fallback file in place by default. Only create or migrate to root \`AGENTS.md\` if the user explicitly asks to migrate.
...
For projects with multiple concerns, suggest organizing instructions into \`.neuron/rules/\` as separate focused files (e.g., \`code-style.md\`, \`testing.md\`, \`security.md\`). These are loaded automatically alongside AGENTS.md and can be scoped to specific file paths using \`paths\` frontmatter.
...
## Phase 5: Write NEURON.local.md (if user chose personal or both)

Write a minimal NEURON.local.md at the project root. This file is automatically loaded alongside AGENTS.md. After creating it, add \`NEURON.local.md\` to the project's .gitignore so it stays private.

**Consume \`note\` entries from the Phase 3 preference queue whose target is NEURON.local.md** (personal-level notes) — add each as a concise line. If the user chose personal-only in Phase 1, this is the sole consumer of note entries.
...
If Phase 2 found multiple git worktrees and the user confirmed they use sibling/external worktrees (not nested inside the main repo): the upward file walk won't find a single NEURON.local.md from all worktrees. Write the actual personal content to \`~/.neuron/<project-name>-instructions.md\` and make NEURON.local.md a one-line stub that imports it: \`@~/.neuron/<project-name>-instructions.md\`. The user can copy this one-line stub to each sibling worktree. Never put this import in the project AGENTS.md. If worktrees are nested inside the main repo (e.g., \`.neuron/worktrees/\`), no special handling is needed — the main repo's NEURON.local.md is found automatically.

If NEURON.local.md or legacy CLAUDE.local.md already exists: read it, propose specific additions, and do not silently overwrite.
...
If \`.neuron/skills/\` already exists with skills, review them first. Do not overwrite existing skills — only propose new ones that complement what is already there.

Create each skill at \`.neuron/skills/<skill-name>/SKILL.md\`:
...
- **Proposal-sourced hooks** (if user chose "Skills + hooks" or "Hooks only"): Consume \`hook\` entries from the Phase 3 preference queue. If Phase 2 found a formatter and the queue has no formatting hook, offer format-on-edit as a fallback. If the user chose "Neither" or "Skills only" in Phase 1, skip this bullet entirely.
  ...
  1. Target file: default based on the Phase 1 instruction-file choice — project → \`.neuron/settings.json\` (team-shared, committed); personal → \`.neuron/settings.local.json\`. Only ask if the user chose "both" in Phase 1 or the preference is ambiguous. Ask once for all hooks, not per-hook.
  ...
  3. **Load the hook reference** (once per \`/init\` run, before the first hook): invoke the Skill tool with \`skill: 'update-config'\` and args starting with \`[hooks-only]\` followed by a one-line summary of what you're building — e.g., \`[hooks-only] Constructing a PostToolUse/Write|Edit format hook for .neuron/settings.json using ruff\`. This loads the hooks schema and verification flow into context. Subsequent hooks reuse it — don't re-invoke.
