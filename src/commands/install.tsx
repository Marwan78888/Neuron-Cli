...
import { formatCliCommand, getCliDisplayName } from '../utils/cliIdentity.js';
...
export function getInstallationPath(): string {
  ...
  if (isWindows) {
    ...
    const windowsPath = join(homeDir, '.local', 'bin', 'neuron.exe');
    // Replace forward slashes with backslashes for Windows display
    return windowsPath.replace(/\//g, '\\');
  }
  return '~/.local/bin/neuron';
}
...
function Install({
    ...
    async function run() {
      try {
        ...
        if (result.lockFailed) {
          throw new Error('Could not install - another process is currently installing Neuron. Please try again in a moment.');
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, `${getCliDisplayName()} installation completed successfully`, {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, `${getCliDisplayName()} installation failed`, {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">
          Installing {getCliDisplayName()} native build {state.version}...
        </Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              {getCliDisplayName()} successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install Neuron native build',
