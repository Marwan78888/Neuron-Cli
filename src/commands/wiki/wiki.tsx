...
Manage the Neuron project wiki stored in .neuron/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized Neuron wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `Neuron wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the Neuron wiki.`,
    '',
    `Title: ${result.title}`,
